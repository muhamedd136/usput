...
const Orders = ({ update }) => {
  ...
  const fetchAppliedOffers = async () => {
    await order
      ...
      .catch(() => console.log("Can't fetch applied offers orders."));
  };
  ...
  const fetchRequestedOffers = async () => {
    await order
      ...
      .catch(() => console.log("Can't fetch requested offers orders."));
  };
  ...
  const fetchCompletedOffers = async () => {
    await order
      ...
      .catch(() => console.log("Could not get completed offers, try again."));
  };
